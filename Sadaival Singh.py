"""
    from  https://www.youtube.com/watch?v=v-XcmsYlzjA
"""
...
def define_regon(frame):
    cv2.rectangle(frame, (100, 100), (300, 300), (0, 255, 0), 0)
    return frame[100:300, 100:300]


def image_processing(roi):
...
def draw_contours(roi,approx,defects):
    l=0
...
def detect_postures(frame,hull,cnt,l):
...
while (1):
    ...
    try:  # an error comes if it does not find anything in window as it cannot find contour of max area
        ...
        roi = define_regon(frame)
        hsv = cv2.cvtColor(roi, cv2.COLOR_BGR2HSV)

        mask = image_processing(roi)
    ...
    # code for finding no. of defects due to fingers
        l=draw_contours(roi,approx,defects)
        # print corresponding gestures which are in their ranges
        detect_postures(frame,hull,cnt,l)
        # show the windows
